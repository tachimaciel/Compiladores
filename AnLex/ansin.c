 ...
 *
 *	Descripción: 
 *     Implementación de un Analizador Sintáctico para el lenguaje JSON simplificado.	
 *	   
 *  AUTORES
 *	- EDUARDO MACIEL
 *  - JUAN PABLO MARTINEZ
...
{
    int conjPrimero[] = {STRING,R_LLAVE,0,0,0,0,0,0};
    int conjSig[] = {EOF,COMA, R_CORCHETE, R_LLAVE,0,0,0,0};
    checkInput(conjPrimero,conjSig);
...
void atribute()
{
    int conjPrimero[] = {STRING,0,0,0,0,0,0,0};
...
{
    int conjPrimero[] = {L_LLAVE, L_CORCHETE, STRING, NUMBER, PR_TRUE, PR_FALSE, PR_NULL, 0};
    int conjSig[] = {COMA,R_LLAVE,0,0,0,0,0,0};
    checkInput(conjPrimero,conjSig);
...
{
    // Verificar si se pasa el archivo fuente como parametro
    if (argc > 1) 
    {
        ...
        // Se verifica si existen o no errores sintacticos
        if(bandError == 0)
        {
            printf("El fuente es sintácticamente correcto.\n");
        } else
        {
            printf("El fuente no es sintácticamente correcto.\n");
        }
